...
系统 SHALL 提供一个专用 MCP 工具用于查询 `SELECT` 的执行计划（建议工具名 `dm_explain_plan(select_sql)`）。该工具 SHALL 在一次调用中完成计划生成与计划读取/返回，且不得要求用户再通过单独的 `dm_query("SELECT ... FROM PLAN_TABLE ...")` 来拼接两次调用的结果（因为两次调用可能落在不同数据库 session）。实现不得假设目标库的 `PLAN_TABLE` 一定存在 `STATEMENT_ID`，而必须依据目标库实际计划表结构选择兼容的读取策略。
