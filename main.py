...
def add(a: int, b: int) -> dict:
    ...
    Returns:
        dict: 包含计算结果和详细元数据的字典
            成功时返回结构:
            {
                "success": bool,           # 操作是否成功，恒为true
                "result": int,            # 计算结果 (a + b)
                "operation": str,         # 操作类型，恒为"addition"
                "operands": list,         # 操作数列表 [a, b]
                "metadata": dict          # 执行元数据
                {
    ...
    Raises:
        InvalidParameterError: 当参数不是整数时抛出
        Exception: 其他未预期的错误情况

    错误处理说明:
        - 参数验证失败: 返回详细的错误信息，指出具体哪个参数无效
        - 执行异常: 包含异常类型和消息，便于调试
        - 所有错误情况都会记录执行时间和完整的上下文信息
    ...
    try:
        ...
        if not isinstance(a, int) or not isinstance(b, int):
            raise InvalidParameterError("两个参数都必须是整数")
...
def get_greeting(name: str) -> dict:
    ...
    Returns:
        dict: 问候信息，包含：
            - success: 指示问候是否成功生成的布尔值
            - greeting: 个性化问候消息
            - name: 被问候的名字
            - metadata: 包含时间戳的生成信息
            - error: 如果问候生成失败时的错误消息
    ...
    try:
        ...
        if not name or not name.strip():
            raise InvalidParameterError("名字不能为空")

        validated_name = name.strip()

        # 验证名字长度和字符
        if len(validated_name) > 100:
            raise InvalidParameterError("名字过长（最多 100 个字符）")

        # 基本名字验证（允许字母、空格、连字符、撇号）
        if not re.match(r"^[a-zA-Z\s\-']+$", validated_name):
            raise InvalidParameterError("名字包含无效字符")

        # 生成问候
        greeting = f"Hello, {validated_name}!"
        execution_time = time.time() - start_time
...
# 添加一个带有增强功能和验证的提示
@mcp.prompt()
def greet_user(name: str, style: str = "friendly") -> dict:
    """
    生成带有验证和多种样式选项的可定制问候提示。

    Args:
        name: 要问候的人的名字（必需）
        style: 问候样式 - "friendly"（友好）、"formal"（正式）、"casual"（随意）或 "professional"（专业）（默认："friendly"）

    Returns:
        dict: 提示生成结果，包含：
            - success: 指示提示是否成功生成的布尔值
            - prompt: 生成的问候提示
            - name: 使用的名字
            - style: 应用的样式
            - metadata: 包含执行时间的生成信息
            - error: 如果提示生成失败时的错误消息

    Example:
        >>> greet_user("Alice", "formal")
        {
            "success": True,
            "prompt": "Please write a formal, professional greeting for someone named Alice.",
            "name": "Alice",
            "style": "formal",
            "metadata": {
                "timestamp": 1234567890.123,
    ...
    try:
        ...
        styles = {
            ...
            "formal": "请写一个正式、专业的问候",
            "casual": "请写一个随意的、轻松的问候",
            "professional": "请写一个适合商业场合的、尊重的问候"
        }

        # 验证样式参数
        if style not in styles:
            raise InvalidParameterError(f"无效的样式 '{style}'。支持的样式: {', '.join(styles.keys())}")

        # 生成提示
        prompt = f"{styles[style]}，问候对象是 {validated_name}。"
        execution_time = time.time() - start_time
        ...
        return {
            ...
            "metadata": create_response_metadata(
                ...
                success=True,
                execution_time=execution_time,
                additional_info={"style_used": style}
            )
        }
